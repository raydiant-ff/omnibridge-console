...
export default async function CheckoutReturnPage({
  ...
  if (session_id && record.status !== "accepted") {
    try {
      ...
      if (session.status === "complete") {
        ...
        if (record.sfQuoteId) {
          try {
            ...
            console.error("[Checkout Return] SF sync error:", err);
          }
        }
      } else if (session.status === "open") {
        redirect(`/accept/${token}?canceled=true`);
      }
    } catch (err) {
      console.error("[Checkout Return] Session retrieval error:", err);
    }
  }

  if (record.status !== "accepted" && !session_id) {
